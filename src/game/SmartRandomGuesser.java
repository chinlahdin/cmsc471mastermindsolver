...
/**
 * The smart random guesser generates guesses by random selecting a guess and
 * checking it against past feedbacks to see if it is worth asking the oracle
 * for new feedback.
 * 
...
public class SmartRandomGuesser implements Guesser
{
	...
	/**
	 * Resets the feedback and guess arraylist and the color space.
	 * 
	 */
	public void reset()
	{
	...
	/**
	 * The guessing algorithm for randomly generating guesses.
	 * 
	 */
	public CodeSequence guess()
	{
		...
		{
			/*
			 * if(guesses.isEmpty() && theBias.equals("b1")){ int halfWay =
			 * this.nrPegs / 2; int value = 1; for(int y = 0; y <
			 * cheatArray.length; y++){ if( y >= halfWay){ cheatArray[y] =
			 * value++; } else{ cheatArray[y] = value; if( y % 2 == 1){
			 * value+=1; } } } guess = new CodeSequence(cheatArray);
			 * 
			 * } else if(guesses.isEmpty() && theBias.equals("b2")){ cheatArray
			 * = new int[this.nrPegs]; for(int z = 0; z < cheatArray.length;
			 * z++){ cheatArray[z] = 1; }
			 * 
			 * guess = new CodeSequence(cheatArray); } else
			 * if(theBias.equals("b3")){ int offset = new
			 * Random().nextInt(workingColorSpace.length());
			 * 
			 * for(int i = 0; i < nrPegs; i++){ cheatArray[i] =
			 * workingColorSpace.getColor(i + offset); }
			 * 
			 * guess = new CodeSequence(cheatArray); } else
		 ...
		 * " guesses generated-- best guess matches " + ((guesses.size() ==
		 * nrOfGuessMatches) ? "all " + guesses.size() + " previous" :
	...
	/**
	 * Uses past feedback to generate feedback without asking the oracle,
	 * registering an actual guess.
	 * 
	 */
	public void giveFeedbackForLastGuess(Feedback feedback)
	{
		 ...
		 * if(feedback.getBlackAndWhite() == 2 ){ for(int a =
		 * lastGuess.getNrPegs() /2; a < lastGuess.getNrPegs(); a++ ){
		 * workingColorSpace.removeColor(lastGuess.getPegColorAt(a)); } }
		 * if(feedback.getBlackAndWhite() == 1){ for(int a = 0; a <
		 * lastGuess.getNrPegs() / 2; a++){
		 * workingColorSpace.removeColor(lastGuess.getPegColorAt(a)); } } }
		 */
