...
public class RandomGuess extends CodeSequence
{
	...
	public RandomGuess(ColorSpace colors, CodeSequence sequenceToMatch, Feedback feedbackToMatch)
	{
		super(makeRandomGuessFromSequenceFeedback(colors, sequenceToMatch, feedbackToMatch));
	}
	
	private static int[] makeTotallyRandomGuess(ColorSpace possiblePegColors, int nrPegs)
	{
	...
	private static int[] makeRandomPermutationGuess(CodeSequence sequenceToPermute)
	{
	...
	private static int[] makeRandomGuessFromSequenceFeedback(ColorSpace colors, CodeSequence sequenceToMatch, Feedback feedbackToMatch)
	{
		int[] newGuess = new int[sequenceToMatch.getNrPegs()];
		int[] indexesToPermute = createUniqueRandIntSet( feedbackToMatch.getBlack() + feedbackToMatch.getWhite(), sequenceToMatch.getNrPegs());
		...
		//The first indexes will be our black pegs and the map to their own index (identity)
		for( int i = 0; i < feedbackToMatch.getBlack(); i++ )
		{
			while( feedbackToMatch.getWhite() > 0 && unusedColorsAreTheSame(permutation, sequenceToMatch, indexesToPermute[i]) ||
				(i == 0 && indexesToPermute.length == sequenceToMatch.getNrPegs() && sequenceToMatch.getNrPegs() <= maxColorFreq * 2 &&
				 sequenceToMatch.getPegColorAt(indexesToPermute[0]) != mostFreqColor) )
			{
				int temp = indexesToPermute[indexesToPermute.length - 1];
				System.arraycopy(indexesToPermute, 0, indexesToPermute, 1, indexesToPermute.length - 1);
				indexesToPermute[i] = temp;
			}
			permutation[indexesToPermute[i]] = indexesToPermute[i];
		}
				
		//The reset of the indexes will be our white pegs, we must find a mapping such that each mapping is unique and is not an identity
		for( int i = feedbackToMatch.getBlack(); i < indexesToPermute.length; i++ )
			permutation[indexesToPermute[i]] = getIndexNonIdentityMapping(permutation, sequenceToMatch, indexesToPermute[i]);
...
/*	
		...
		System.out.println("RANDOM INDEXES: " + arrayToString(indexesToPermute));
		System.out.println("COLORSPACE BEFORE: " + colorSpace);
*/

		//Reduce the color space by removing colors of pegs that were not chosen as having received feedback
		for( int i = 0; i < permutation.length; i++ )
...
//		System.out.println("COLORSPACE AFTER: " + colorSpace);
		...
		//Fill in remaining newGuess pegs with randomly chosen colored pegs from remaining colorspace choices
...
//		System.out.println("------------------------------------------\n");
	...
	private static boolean unusedColorsAreTheSame( int[] permutation, CodeSequence sequence, int index )
	{
		int firstUnusedIndex = 0;
		int color;
		while( firstUnusedIndex < permutation.length && (permutation[firstUnusedIndex] >= 0 || firstUnusedIndex == index ) )
			firstUnusedIndex++;
		...
		for( int i = firstUnusedIndex; i < permutation.length; i++ )
			if( permutation[i] < 0 && sequence.getPegColorAt(i) != color && i != index )
				return false;
		
		return true;
	}
	
	private static int getIndexNonIdentityMapping( int[] permutation, CodeSequence sequence, int index )
	{
		int indexMapping = -1;
		Random rng = new Random();
    	...
    	{
			//System.out.println( "WHITE MAPPING -- " + arrayToString(permutation) + " -- " + index + " -- " + sequence );
			do { indexMapping = rng.nextInt(permutation.length); } while( indexMapping == index );
    		isValidMapping = true;
	...
	private static int[] amakeRandomGuessFromSequenceFeedback(ColorSpace colors, CodeSequence sequenceToMatch, Feedback feedbackToMatch)
	{
